...
export function normalizeLargeItemSettings(raw = {}) {
  ...
  for (const name of BOOLEANS) if (typeof raw?.[name] === "boolean") settings[name] = raw[name];
  for (const name of COLORS) if (COLOR_PATTERN.test(String(raw?.[name] || ""))) {
    settings[name] = raw[name].toUpperCase();
  }
...
function startInspector(sdk, documentRef) {
  ...
  form.addEventListener("change", event => {
    ...
    } else if (COLORS.includes(source?.name)) form.elements[`${source.name}Hex`].value = source.value;
    const raw = {};
    for (const name of BOOLEANS) raw[name] = Boolean(form.elements[name]?.checked);
    for (const name of COLORS) raw[name] = form.elements[`${name}Hex`]?.value;
