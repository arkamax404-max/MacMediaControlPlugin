...
from largeitem_renderer import LargeItemSettings, LargeItemView, render_largeitem, svg_data_uri
...
def settings_match(raw: object, settings: LargeItemSettings) -> bool:
    if not isinstance(raw, Mapping):
        return False
    return all(raw.get(name) == value
               for name, value in largeitem_settings_payload(settings).items())
...
class LargeItemActionModel:
    ...
    def add(self, event: object) -> tuple[RenderRequest, ...]:
        ...
        with self._lock:
            ...
            entry = _Context(self._next_generation, 1, True, settings,
    ...
    def clear(self, event: object) -> bool:
        ...
        try:
            contexts = tuple(item.get("context") for item in tuple(items)
    ...
    def render(self, request: RenderRequest, media: MediaSnapshot, progress: ProgressState,
        ...
        if bundle is not None and bundle.artwork_id == media.artwork_id:
            artwork = (bundle.color if media.is_playing or settings.paused_artwork == "color"
                       else bundle.grayscale)
        view = LargeItemView(media.status, media.title, media.artist, media.is_playing,
                             artwork, ratio, elapsed, remaining, settings)
        data_uri = svg_data_uri(render_largeitem(view))
        signature = hashlib.sha256(data_uri.encode("ascii")).hexdigest()
        intent = SendIntent(request.context, data_uri, request.generation,
                            request.version, signature)
        with self._lock:
            entry = self._matching(request)
...
def _event_identity(event: object) -> tuple[str, object] | None:
    ...
    return (context, event.get("param")) if key == KEY else None
