...
def render_largeitem(view: LargeItemView) -> str:
    ...
    title_nodes = "".join(
        ...
        f'{_text_constraint(line, title_width, 25)}>{_xml(line)}</text>'
    ...
    return (
        ...
        f'fill="{background}"/>{artwork}{playback}{title_nodes}'
...
def _artwork(value: object, fit: object) -> str:
    ...
    return ('<image x="8" y="8" width="180" height="180" '
            f'preserveAspectRatio="xMidYMid {mode}" href="{value}"/>')


def _fallback_artwork(background: str, secondary: str) -> str:
    return (f'<rect x="8" y="8" width="180" height="180" rx="12" fill="{background}" '
            f'stroke="{secondary}" stroke-width="2"/>'
            f'<path d="M70 70v58c0 13-22 13-22 0s22-13 22 0V82l58-12v46c0 13-22 13-22 0s22-13 22 0V58z" fill="{secondary}"/>')
...
def _progress(view: LargeItemView, x: int, width: int, secondary: str, accent: str) -> str:
    ...
    return (f'{times}<rect x="{x}" y="170" width="{width}" height="8" rx="4" '
            f'fill="{secondary}" opacity="0.35"/><rect x="{x}" y="170" '
            f'width="{_fixed(width * ratio)}" height="8" rx="4" fill="{accent}"/>')


def _safe_text(value: object) -> str:
    raw = str(value or "")
    text = unicodedata.normalize("NFC", "".join(
        "�" if 0xD800 <= ord(character) <= 0xDFFF else character for character in raw
    ))
...
def _text_constraint(value: str, width: int, font_size: int) -> str:
    estimated = sum(
        ...
        .9 if character in "WM@%&" else .3 if character in "ilI.,:;|'" else .56
