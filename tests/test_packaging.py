...
def load_preparer():
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError("Could not load prepare_ulanzi_spike.py")
...
def create_runtime(root, preparer):
    for relative in preparer.REQUIRED_RUNTIME_FILES:
        ...
        path.write_bytes(
            struct.pack("<IiiIIIII", 0xFEEDFACF, CPU_TYPE_X86_64, 3, 2, 0, 0, 0, 0)
            if relative in {"MediaControlRuntime", "MediaRemoteHelper"}
            else b"license"
        )
...
def trusted_x86_64_lipo(command, **_kwargs):
...
def add_pyinstaller_macos_dylib_alias(
    root, name="libXau.6.dylib", target="PIL/.dylibs/libXau.6.dylib"
):
...
class PackagingContractTests(unittest.TestCase):
    def test_release_metadata_is_consistently_v2_4_0(self):
        ...
        setup_action = (plugin / "runtime" / "python" / "setup_action.py").read_text(
            "utf-8"
        )
        ...
        self.assertEqual(
            companion_version.splitlines()[0], 'COMPANION_VERSION = "2.4.0"'
        )
    ...
    def test_macos_build_contract_uses_extensionless_launcher_runtime(self):
        ...
        launcher = (
            ROOT
            / "com.arkamax404.mediacontrold200.ulanziPlugin"
            / "src"
            / "launcher.js"
        ).read_text("utf-8")
    ...
    def test_companion_import_graph_has_hashed_runtime_and_bundle_contracts(self):
        ...
        lock = (PACKAGING / "requirements-ulanzi-runtime.lock").read_text("utf-8")
        pillow = next(
            line for line in lock.splitlines() if line.startswith("Pillow==11.3.0 ")
        )

        self.assertIn("from PIL import Image, ImageOps", artwork)
        self.assertRegex(
            lock, r"(?m)^Pillow==[^\s]+(?:\s+--hash=sha256:[0-9a-f]{64})+$"
        )
        self.assertIn(
    ...
    def test_mediaremote_helper_builder_uses_fixed_native_inputs(self):
        ...
        if spec is None or spec.loader is None:
            raise RuntimeError("Could not load build_mediaremote_helper.py")
    ...
    def test_cpython_runtime_license_is_repository_contained_and_attributed(self):
        ...
        self.assertEqual(
            sha256(normalized_content).hexdigest(),
    ...
    @unittest.skipUnless(os.name == "posix", "requires POSIX executable mode semantics")
    def test_runtime_inventory_is_deterministic_and_macos_only(self):
        preparer = load_preparer()
        with tempfile.TemporaryDirectory() as directory:
            runtime = Path(directory)
            create_runtime(runtime, preparer)
            first = preparer.validate_runtime_bundle(
                runtime, architecture_runner=trusted_x86_64_lipo
            )
            self.assertEqual(
                first,
                preparer.validate_runtime_bundle(
                    runtime, architecture_runner=trusted_x86_64_lipo
                ),
            )
    ...
    @unittest.skipUnless(os.name == "posix", "requires POSIX symbolic link semantics")
    def test_runtime_inventory_allows_only_contained_pyinstaller_python_symlink_closures(
        self,
    ):
        preparer = load_preparer()
        for name, version in (("Python3", "3.9"), ("Python", "3.13")):
            with (
                ...
                self.assertEqual(
                    ...
                    tuple(
                        sorted(
                            preparer.REQUIRED_RUNTIME_FILES
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            create_runtime(runtime, preparer)
            os.symlink(
                "../MediaControlRuntime", runtime / "_internal" / "unexpected-link"
            )
            with self.assertRaisesRegex(ValueError, "symbolic link"):
                preparer.validate_runtime_bundle(runtime)
        with tempfile.TemporaryDirectory() as directory:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            os.symlink(
                "Python.framework/Versions/3.13/Python.exe",
                runtime / "_internal" / "Python",
            )
            with self.assertRaisesRegex(ValueError, "unexpected target"):
                preparer.validate_runtime_bundle(runtime)
    ...
    def test_runtime_inventory_rejects_unsafe_pyinstaller_dylib_aliases(self):
        preparer = load_preparer()
        cases = (
            ("libXau.6.dylib", "/not-contained/libXau.6.dylib", None),
        ...
        for name, target, content in cases:
            with (
                self.subTest(name=name, target=target),
                tempfile.TemporaryDirectory() as directory,
            ):
                runtime = Path(directory)
                create_runtime(runtime, preparer)
                ...
                with self.assertRaisesRegex(
                    ValueError, "symbolic link|escapes the runtime root"
                ):
                    preparer.validate_runtime_bundle(runtime)
    ...
    def test_projection_preserves_manifest_actions_assets_and_inspector(self):
        ...
        protected = {
            ...
            for name in ("manifest.json", "package.json", "src/app.js", "src/plugin.js")
        }
        with (
            ...
            with patch.object(
                ...
                return_value=tuple(sorted(preparer.REQUIRED_RUNTIME_FILES)),
            ):
                target = preparer.prepare_package(
                    plugin, runtime, Path(output_dir), ROOT
                )
            ...
            self.assertEqual(manifest["Banner"], ["assets/banners/banner-1.png"])
            self.assertEqual(manifest["Banner"], source_manifest["Banner"])
            self.assertEqual(
                (target / "assets" / "banners" / "banner-1.png").read_bytes(),
                (plugin / "assets" / "banners" / "banner-1.png").read_bytes(),
            )
            self.assertEqual(
                [action["UUID"] for action in manifest["Actions"]],
                [
            ...
            self.assertEqual(
                len(manifest["Actions"]), len(preparer.PORTED_ACTION_SUFFIXES)
            )
            self.assertTrue(
                all(
            ...
            self.assertEqual(
                protected, {name: (plugin / name).read_bytes() for name in protected}
            )

    @unittest.skipUnless(
        sys.platform == "darwin", "requires macOS executable/lipo semantics"
    )
        ...
        for name in ("MediaControlRuntime", "MediaRemoteHelper"):
            with self.subTest(name=name), tempfile.TemporaryDirectory() as directory:
                ...
                (runtime / name).write_bytes(
                    struct.pack(
                        "<IiiIIIII", 0xFEEDFACF, CPU_TYPE_ARM64, 0, 2, 0, 0, 0, 0
                    )
                )
                with self.assertRaisesRegex(
                    ValueError, f"{name} is not x86_64-compatible"
                ):
                    preparer.validate_runtime_bundle(runtime)
        ...
        for name in ("MediaControlRuntime", "MediaRemoteHelper"):
            for shape, byte_order, magic, cpu_type, header_size in cases:
                for truncated_size in (8, header_size - 1):
                    with (
                        self.subTest(name=name, shape=shape, size=truncated_size),
                        tempfile.TemporaryDirectory() as directory,
                    ):
                        runtime = Path(directory)
                        create_runtime(runtime, preparer)
                        truncated_header = struct.pack(
                            f"{byte_order}II", magic, cpu_type
    ...
    def test_runtime_fails_closed_when_lipo_cannot_produce_trusted_architectures(self):
        ...
        failures = (
            (
                ...
                "Could not run /usr/bin/lipo.*missing",
            ),
            (
                "raises",
                RuntimeError("runner failed"),
                "Could not run /usr/bin/lipo.*runner failed",
            ),
            (
                "timeout",
                subprocess.TimeoutExpired(preparer.LIPO, 10),
                "/usr/bin/lipo timed out",
            ),
            (
                "nonzero",
                subprocess.CompletedProcess([], 7, "", "invalid binary"),
            ...
            (
                "unexpected",
                subprocess.CompletedProcess([], 0, "x86_64 mystery\n", ""),
                "lipo reported malformed architectures.*x86_64 mystery",
            ),
            (
                "duplicate",
                subprocess.CompletedProcess([], 0, "x86_64 x86_64\n", ""),
        ...
        for name in ("MediaControlRuntime", "MediaRemoteHelper"):
            for case, failure, message in failures:
                with (
                    self.subTest(name=name, case=case),
                    tempfile.TemporaryDirectory() as directory,
                ):
                    runtime = Path(directory)
                    create_runtime(runtime, preparer)
                    ...
                    def runner(
                        command, *, expected_name=name, failure=failure, **kwargs
                    ):
                        if Path(command[-1]).name == expected_name:
                    ...
                    with self.assertRaisesRegex(ValueError, message):
                        preparer.validate_runtime_bundle(
                            runtime, architecture_runner=runner
                        )
    ...
    @unittest.skipUnless(os.name == "posix", "requires POSIX executable mode semantics")
    def test_runtime_invokes_absolute_lipo_without_a_shell_for_each_required_executable(
        self,
    ):
        preparer = load_preparer()
        with tempfile.TemporaryDirectory() as directory:
            ...
            create_runtime(runtime, preparer)
            runner = Mock(
                return_value=subprocess.CompletedProcess([], 0, "x86_64\n", "")
            )
            ...
            for call, name in zip(
                ...
                self.assertEqual(
                    call.args[0], ["/usr/bin/lipo", "-archs", str(runtime / name)]
                )
                self.assertEqual(
                    call.kwargs,
                    {
    ...
    def test_runtime_accepts_universal_lipo_output_with_x86_64(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            create_runtime(runtime, preparer)
            runner = Mock(
                return_value=subprocess.CompletedProcess([], 0, "x86_64 arm64\n", "")
            )
            self.assertEqual(
                preparer.validate_runtime_bundle(runtime, architecture_runner=runner),
                tuple(sorted(preparer.REQUIRED_RUNTIME_FILES)),
    ...
    def test_projection_rejects_non_macos_manifest(self):
        preparer = load_preparer()
        manifest = json.loads(
            (ROOT / preparer.PLUGIN_FOLDER / "manifest.json").read_text("utf-8")
        )
        ...
        with (
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-B",
                    str(PACKAGING / "prepare_ulanzi_spike.py"),
                    "--runtime-bundle",
                    str(runtime),
                    "--output-root",
                    output_dir,
