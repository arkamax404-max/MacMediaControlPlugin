...
class MacOSCurrentMediaAdapterTests(unittest.IsolatedAsyncioTestCase):
    ...
    def _processor():
        ...
        processor._decode_encode = Mock(
            return_value=ArtworkVariants(PNG_URI, PNG_URI, (PNG_URI,) * 4)
        )
        return processor
    ...
    async def test_refresh_retains_playing_timeline_anchor_when_mediaremote_position_is_unchanged(
        self,
    ):
        first = datetime(2026, 9, 3, 12, 0, tzinfo=timezone.utc)
        second = datetime(2026, 9, 3, 12, 0, 5, tzinfo=timezone.utc)
        gateway = Mock(
            read_now_playing=Mock(
                side_effect=[
        ...
        adapter = MacOSCurrentMediaAdapter(
            cache, gateway=gateway, clock=self._clock(first, second)
        )
    ...
    async def test_refresh_uses_valid_mediaremote_timeline_anchor(self):
        ...
        adapter = MacOSCurrentMediaAdapter(
            cache,
            gateway=Mock(
                read_now_playing=Mock(return_value={"status": "ok", "media": media})
            ),
    ...
    async def test_refresh_reanchors_on_playback_state_change(self):
        ...
        cache = MediaStateCache()
        adapter = MacOSCurrentMediaAdapter(
            cache, gateway=gateway, clock=self._clock(first, second, third)
        )
    ...
    async def test_refresh_reanchors_after_media_becomes_unavailable(self):
        ...
        cache = MediaStateCache()
        adapter = MacOSCurrentMediaAdapter(
            cache, gateway=gateway, clock=self._clock(first, third)
        )
    ...
    async def test_refresh_processes_generic_artwork_bytes_through_existing_cache_contract(
        self,
    ):
        cache, processor = MediaStateCache(), self._processor()
        gateway = Mock(
            read_now_playing=Mock(return_value={"status": "ok", "media": self._media()})
        )
        adapter = MacOSCurrentMediaAdapter(
            cache, gateway=gateway, artwork_processor=processor
        )
        ...
        self.assertNotIn(
            base64.b64encode(ARTWORK).decode(), json.dumps(cache.get().public())
        )
    ...
    async def test_refresh_is_off_event_loop_and_unavailable_data_fails_closed(self):
        ...
        adapter = MacOSCurrentMediaAdapter(
            cache,
            gateway=Mock(read_now_playing=reader),
            clock=lambda: datetime(2026, 9, 3, tzinfo=timezone.utc),
        )
        with patch(
            "d200_bridge.macos_media.asyncio.to_thread",
    ...
    async def test_transport_is_generic_current_source_and_preserves_adapter_operations(
        self,
    ):
        cache = MediaStateCache()
        ...
        adapter = MacOSCurrentMediaAdapter(
            ...
            gateway=Mock(
                read_now_playing=Mock(
                    return_value={"status": "ok", "media": self._media()}
                ),
                command=command,
            ),
        )
        ...
        self.assertFalse(cache.get().available)
        self.assertEqual(
            command.call_args_list, [(("previous",),), (("toggle",),), (("next",),)]
        )

    async def test_invalid_artwork_and_unknown_media_state_do_not_publish_stale_media(
        self,
    ):
        cache = MediaStateCache()
        gateway = Mock(
            read_now_playing=Mock(
                return_value={
                    "status": "ok",
                    "media": {**self._media(), "state": "buffering", "artwork": b"bad"},
                }
            )
        )
        ...
        runner = Mock(
            return_value=Mock(
                ...
                stdout=json.dumps(
                    {
                        "status": "ok",
                        "media": {**self._media(), "artwork": encoded},
                    }
                ),
            )
        )

        result = MediaRemoteGateway(
            runner=runner, helper_path="MediaRemoteHelper"
        ).read_now_playing()
    ...
    def test_mediaremote_gateway_preserves_rejected_transport_protocol(self):
        ...
        gateway = MediaRemoteGateway(
            runner=Mock(
                return_value=Mock(returncode=0, stdout='{"status":"rejected"}')
            ),
    ...
    def test_mediaremote_gateway_reports_bounded_redacted_failure_diagnostics(self):
        ...
        sensitive_payload = "Sensitive title /Users/person/token artwork-bytes"
        cases = (
            (Mock(side_effect=FileNotFoundError(sensitive_payload)), "launch", None),
            (
                Mock(
                    side_effect=subprocess.TimeoutExpired(
                        "MediaRemoteHelper", 3, output=sensitive_payload
                    )
                ),
                "timeout",
                None,
            ...
            (
                Mock(
                    return_value=Mock(
                        returncode=78,
                        stdout=sensitive_payload,
                        stderr=sensitive_payload,
                    )
                ),
                "exit",
                78,
            ...
            (
                Mock(
                    return_value=Mock(
                        returncode=0, stdout="not-json", stderr=sensitive_payload
                    )
                ),
                "parse",
                None,
            ...
            (
                Mock(
                    return_value=Mock(
                        returncode=0,
                        stdout='{"status":"ok","media":{"title":"T"}}',
                        stderr=sensitive_payload,
                    )
                ),
                "schema",
        ...
        for runner, stage, exit_code in cases:
            with self.subTest(stage=stage):
                ...
                self.assertEqual(
                    gateway.read_now_playing(), {"status": "unavailable", "media": None}
                )
                self.assertEqual(
                    gateway.last_diagnostic, {"stage": stage, "exit_code": exit_code}
                )
                self.assertNotIn(sensitive_payload, repr(gateway.last_diagnostic))
    ...
    def test_mediaremote_diagnostic_log_never_contains_media_or_paths(self):
        ...
        runner = Mock(
            return_value=Mock(
                returncode=78, stdout="Title", stderr="/Users/person/token"
            )
        )
        logger = Mock()
        with patch(
            "d200_bridge.macos_mediaremote.logging.getLogger", return_value=logger
        ):
            MediaRemoteGateway(
                runner=runner, helper_path="MediaRemoteHelper"
            ).read_now_playing()
        ...
        record = logging.LogRecord(
            "d200_bridge", logging.INFO, "", 0, message, (stage, code), None
        )
    ...
    def test_mediaremote_diagnostic_log_formats_without_safe_event_filter(self):
        from d200_bridge.macos_mediaremote import MediaRemoteGateway
        ...
        self.assertIn("INFO mediaremote_failure stage=exit exit=78", output)
        self.assertNotIn("Title", output)
        self.assertNotIn("/Users", output)

    @unittest.skipUnless(
        sys.platform == "darwin", "MediaRemote helper seam requires macOS Foundation"
    )
    def test_native_transport_waits_for_observable_effect_and_never_emits_media(self):
        source = (
            Path(__file__).parents[1] / "d200_bridge" / "native" / "MediaRemoteHelper.m"
        )
        fake = r"""
...
"""
        with tempfile.TemporaryDirectory() as directory:
            fake_path, helper = (
                Path(directory) / "fake.m",
                Path(directory) / "MediaRemoteHelper",
            )
            ...
            delayed = subprocess.run(
                [str(helper), "toggle"],
                env={**os.environ, "MEDIAREMOTE_HELPER_TEST_SCENARIO": "delayed"},
                check=True,
                capture_output=True,
                text=True,
                timeout=3,
            ...
            no_effect = subprocess.run(
                [str(helper), "next"],
                env={**os.environ, "MEDIAREMOTE_HELPER_TEST_SCENARIO": "none"},
                check=True,
                capture_output=True,
                text=True,
                timeout=3,
