...
from d200_bridge.macos_audio import AudioCommandResult
...
class ShutdownTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_bridge_cleans_up_after_shutdown_notification(self):
        ...
        with patch.object(bridge_main, "create_media_adapter", return_value=adapter) as create_adapter, patch.object(
            bridge_main, "create_audio_controller", return_value=audio
        ), patch.object(
        ...
        create_adapter.assert_called_once_with(ANY)
    ...
    async def test_acquisition_matrix_cleans_every_resource_once(self):
        ...
        for position, failure in enumerate(stages):
            ...
            replacements = dict(MediaStateCache=Mock(return_value=cache), create_media_adapter=Mock(side_effect=fail if position == 0 else None, return_value=adapter), create_audio_controller=Mock(side_effect=fail if position == 2 else None, return_value=audio), create_server=Mock(side_effect=fail if position == 4 else None, return_value=server), install_signal_handlers=Mock(return_value={}), restore_signal_handlers=Mock())
