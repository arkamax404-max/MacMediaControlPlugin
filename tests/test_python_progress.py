...
class PythonProgressTests(unittest.TestCase):
    ...
    def test_one_poll_and_fetch_drive_nowplaying_and_all_mosaic_tiles(self):
        artwork_id = "6" * 64
        bundle = ArtworkBundle(artwork_id, COLOR_URI, GRAYSCALE_URI,
                               (COLOR_URI, GRAYSCALE_URI, COLOR_URI, GRAYSCALE_URI))
        payload = state(updated_at=NOW.isoformat(), position_updated_at=NOW.isoformat(),
    ...
    def test_control_icon_settings_persist_per_context(self):
        class Api:
            def __init__(self): self.settings = []
            def setSettings(self, settings, context):
                time.sleep(.05)
                self.settings.append((context, settings))
                return len(self.settings) >= 3
            def setPathIcon(self, *_args): return True
            def setBaseDataIcon(self, *_args): return True
        ...
        self.assertTrue(scheduler.handle_property_settings({
            "context": "previous", "param": {"iconColor": "#abcdef"},
        }))
        self.assertLess(time.monotonic() - started, .03,
                        "SDK settings callbacks must not perform persistence I/O")
        self.assertTrue(self._wait_for(lambda: len(api.settings) == 3, 1))
        self.assertEqual(api.settings, [("previous", {"iconColor": "#ABCDEF"})] * 3)
        self.assertEqual(model.context("previous").icon_color, "#ABCDEF")
        self.assertTrue(scheduler.handle_add({
            "uuid": NOW_PLAYING_UUID, "context": "cover",
            "param": {"showProgress": True, "accentColor": "#1DB954"},
        }))
        self.assertTrue(scheduler.handle_property_settings({
            "context": "cover", "param": {"showProgress": False, "accentColor": "#abcdef"},
        }))
        self.assertTrue(self._wait_for(
            lambda: api.settings[-1:] == [("cover", {
        ...
        self.assertTrue(scheduler.handle_add({
            ...
            "param": {"secondaryAction": "none", "badgeColor": "#1DB954"},
        }))
        self.assertTrue(scheduler.handle_property_settings({
            "context": "tile", "param": {
                "secondaryAction": "volume-down", "badgeColor": "#abcdef",
            },
        }))
        self.assertTrue(self._wait_for(
            lambda: api.settings[-1:] == [("tile", {
                "secondaryAction": "volume-down", "badgeColor": "#ABCDEF",
            })], 1))
        self.assertEqual(model.context("tile").secondary_action, "volume-down")
        self.assertTrue(scheduler.stop(.5))
    ...
    def test_nowplaying_progress_advances_on_ticks_without_extra_polls(self):
        ...
        scheduler.start()
        scheduler.handle_add({
            "uuid": NOW_PLAYING_UUID, "context": "cover", "param": {"showProgress": True},
        })
    ...
    def test_nowplaying_without_artwork_does_not_schedule_progress_ticks(self):
        artwork_id = "4" * 64
        payload = state(updated_at=NOW.isoformat(), position_updated_at=NOW.isoformat(),
                        position_seconds=30, artwork_id=artwork_id)
        ...
        class Api:
            ...
            def setBaseDataIcon(self, *_args): return True
            def setPathIcon(self, context, image, text):
                self.paths.append((context, image, text)); return True

        client, api = Client(), Api()
        scheduler = ProgressScheduler(api, client, ProgressActionModel(),
                                      NowPlayingActionModel(), ArtworkBundleCache(),
                                      clock=lambda: NOW, poll_interval=5, tick_interval=.02)
        scheduler.start()
        scheduler.handle_add({
            "uuid": NOW_PLAYING_UUID, "context": "cover", "param": {"showProgress": True},
        })
        self.assertTrue(self._wait_for(lambda: api.paths, 1))
        time.sleep(.08)
        self.assertEqual(client.state_calls, 1)
        self.assertEqual(api.paths, [("cover", "./assets/music.svg", "Playing")])
        self.assertTrue(scheduler.stop(.5))
            ...
            for install_first in (False, True):
                with self.subTest(invalidation=invalidation, install_first=install_first):
                    ...
                    if invalidation == "two-context" and install_first:
                        self.assertEqual(api.sends, [("steady", COLOR_URI)])
                    else:
