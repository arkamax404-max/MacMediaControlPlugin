...
class FoundationTests(unittest.TestCase):
    @unittest.skipUnless(os.name == "posix", "fcntl locking requires a POSIX host")
    def test_macos_paths_and_nonblocking_lock_use_application_support(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                paths.root,
                root / "Library" / "Application Support" / "GSMTCD200Controller",
            )
            self.assertEqual(
                paths.cache, root / "Library" / "Caches" / "GSMTCD200Controller"
            )
            self.assertEqual(
                paths.logs, root / "Library" / "Logs" / "GSMTCD200Controller"
            )
    ...
    def test_posix_lock_closes_the_opened_handle_when_acquisition_errors(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                patch("fcntl.flock", side_effect=OSError("lock unavailable")),
            ):
    ...
    @unittest.skipUnless(
        os.name == "posix", "symlinked home semantics require a POSIX host"
    )
    ...
    def test_paths_are_absolute_normalized_and_fail_closed(self):
        ...
        security = Mock()
        security.validate_chain.side_effect = OSError("unsafe ancestor")
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaises(OSError):
                CompanionPaths.from_environment(
                    {"HOME": str(Path(directory).resolve())}, security
                )

    def test_metadata_rejects_symlinks_and_hardlinks(self):
        security = PathSecurity()
        regular = SimpleNamespace(
            st_mode=stat.S_IFREG, st_nlink=1, st_size=43, st_file_attributes=0
        )
        security.validate_metadata(regular, "file")
        for changes in ({"st_mode": stat.S_IFLNK}, {"st_nlink": 2}):
            with self.assertRaises(OSError):
                security.validate_metadata(
                    SimpleNamespace(**{**regular.__dict__, **changes}), "file"
                )
    ...
    def test_token_creation_reuse_cli_retry_and_cleanup(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "d200_bridge.paths.secrets.token_bytes", return_value=b"\0" * 32
            ):
            ...
            with (
                patch(
                    "d200_bridge.paths.load_token_file",
                    side_effect=[FileNotFoundError, TOKEN],
                ),
    ...
    def test_health_is_bounded_versioned_and_private(self):
        ...
        self.assertEqual(
            (health["api_major"], health["api_minor"]), (API_MAJOR, API_MINOR)
        )
        self.assertEqual(
            (health["companion_version"], health["status"]),
            (COMPANION_VERSION, "degraded"),
        )
    ...
    def test_logging_owns_only_its_handlers_bounds_records_and_rotates(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            configure_logging(
                Path(directory), token=TOKEN, console=False, max_bytes=256
            )
            ...
            try:
                ...
                for _index in range(40):
                    child.error(
                        "Authorization: Bearer %s C:\\Users\\Synthetic Name\\cover.png %s",
                        TOKEN,
                        "synthetic-marker " * 1000,
                        exc_info=True,
                    )
            ...
            self.assertNotIn("Synthetic Name", output)
    ...
    def test_main_acquisition_matrix_releases_lock_before_bridge(self):
        for failure in ("mutex", "paths", "token", "logging"):
            mutex = Mock()
            mutex.acquire.return_value = failure != "mutex"
            error = RuntimeError("failed")
            with (
                patch.object(bridge_main, "create_lock", return_value=mutex),
                patch.object(
                    bridge_main.CompanionPaths,
                    "from_environment",
                    side_effect=error if failure == "paths" else None,
                    return_value=SimpleNamespace(
                        token=Path("C:/token"), logs=Path("C:/logs")
                    ),
                ),
                patch.object(
                    bridge_main,
                    "ensure_token",
                    side_effect=error if failure == "token" else None,
                    return_value=TOKEN,
                ),
                patch.object(
                    bridge_main,
                    "configure_logging",
                    side_effect=error if failure == "logging" else None,
                ),
    ...
    def test_startup_failure_marker_is_fixed_and_categorized(self):
        ...
        self.assertEqual(
            bridge_main.startup_failure_marker(
                RuntimeError("synthetic /private/token")
            ),
    ...
    def test_preflight_loads_dependencies_without_starting_runtime_services(self):
        with (
            patch.object(bridge_main, "load_bridge_dependencies") as load,
            patch.object(bridge_main.asyncio, "run") as run,
...
class ServerSecurityTests(unittest.TestCase):
    def setUp(self):
        ...
        self.lifecycle = CompanionLifecycle()
        self.artwork_lookup = Mock()
        self.request_stop = Mock(
            side_effect=lambda: self.loop.call_soon_threadsafe(self.stop_event.set)
        )
        ...
        self.server = create_server(
            ...
            artwork_lookup=self.artwork_lookup,
    ...
    def raw_command(self, values, instance=True):
        connection = http.client.HTTPConnection(
            BRIDGE_HOST, self.server.server_port, timeout=2
        )
        ...
        if instance is not None:
            connection.putheader(
                ...
                self.lifecycle.instance_id if instance is True else instance,
            )
    ...
    def test_health_and_strict_generic_authorization_matrix(self):
        ...
        self.assertEqual(
            self.raw_command([f"Bearer {TOKEN}"] * 2), (403, {"error": "unauthorized"})
        )
        for path, method in (
            ("/health", "GET"),
            ("/state", "GET"),
            ("/artwork/" + "a" * 64, "GET"),
            ("/lifecycle/stop", "POST"),
            ("/command/unknown", "POST"),
        ):
            with self.assertRaises(HTTPError) as error:
                self.request(path, method)
            error.exception.close()
        self.assertEqual(self.commands, [])
        self.artwork_lookup.assert_not_called()
    ...
    def test_command_instance_mismatch_is_generic_and_never_calls_callback(self):
        for instance in (None, "bad", "123e4567-e89b-42d3-a456-426614174000"):
            self.assertEqual(
                self.raw_command([f"Bearer {TOKEN}"], instance),
    ...
    def test_exact_auth_stop_order_and_command_refusal(self):
        ...
        for instance in (None, "wrong", "123e4567-e89b-42d3-a456-426614174000"):
            ...
            self.assertEqual(
                json.load(error.exception), {"error": "companion_mismatch"}
            )
            error.exception.close()
        ...
        try:
            ...
            self.server.RequestHandlerClass._json = original
        self.assertEqual(self.lifecycle.status, "stopping")
        self.request_stop.assert_called_once_with()
        with self.assertRaises(HTTPError) as error:
