...
class DiagnosticsTests(unittest.TestCase):
    def test_bundle_schema_order_bounds_and_privacy(self):
        with tempfile.TemporaryDirectory() as directory:
            paths = CompanionPaths.from_environment({"HOME": directory})
            ...
            result = create_diagnostics(paths=paths, clock=lambda: NOW, http_get=http_get,
                ...
                dependency_provider=lambda name: "C:\\Synthetic\\metadata" if name == "psutil" else "1.2.3")
            ...
            self.assertEqual(dependencies["psutil"], "unavailable")
            self.assertEqual(dependencies["Pillow"], "1.2.3")
...
os.environ["HOME"] = r"{directory}"
blocked={{"d200_bridge.server"}}
