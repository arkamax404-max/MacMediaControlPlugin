...
RUNTIME_ROOT_ENTRIES = frozenset(
    ("MediaControlRuntime", "MediaRemoteHelper", "_internal")
)
...
def exact_source_path(root, reference, prefix):
    ...
    for part in path.parts:
        ...
        if len(matches) != 1:
            raise ValueError(
                f"Package path is missing or has incorrect case: {reference}"
            )
        current = matches[0]
...
def validate_runtime_symlink(root, candidate, relative):
    ...
    if expected is not None:
        ...
        if candidate.readlink().as_posix() != target:
            raise ValueError(
                f"PyInstaller symbolic link has an unexpected target: {relative}"
            )
    else:
        path = PurePosixPath(relative)
        target = candidate.readlink()
        if (
            path.parent.as_posix() != "_internal"
            or not path.name.startswith("lib")
            or not path.name.endswith(".dylib")
    ...
    if expects_file and not stat.S_ISREG(resolved.stat().st_mode):
        raise ValueError(
            f"PyInstaller symbolic link must resolve to a file: {relative}"
        )
    if not expects_file and not resolved.is_dir():
        raise ValueError(
            f"PyInstaller symbolic link must resolve to a directory: {relative}"
        )
...
def validate_runtime_architecture(runtime_bundle, runner=subprocess.run):
    ...
    for name in required_executables:
        ...
        if not isinstance(result.stdout, str) or not result.stdout.strip():
            raise ValueError(
                f"{LIPO} reported no architectures for required executable {name}"
            )
...
def validate_no_symlinks(root):
    ...
    if links:
        raise ValueError(
            f"Projected package contains symbolic links: {', '.join(links)}"
        )
...
def validate_runtime_bundle(runtime_bundle, architecture_runner=subprocess.run):
    ...
    windows_files = [
        name for name in files if name.lower().endswith(WINDOWS_RUNTIME_SUFFIXES)
    ]
    if windows_files:
...
def validate_source_manifest(manifest):
    ...
    platforms = tuple(
        item.get("Platform")
        for item in manifest.get("OS", [])
        if isinstance(item, dict)
    )
    if platforms != ("macos",):
        raise ValueError("Source manifest must declare only macOS")
    expected = tuple(
        f"{manifest.get('UUID')}.{suffix}" for suffix in PORTED_ACTION_SUFFIXES
    )
...
def prepare_package(plugin_source, runtime_bundle, output_root, repo_root):
    ...
    package = read_json(plugin_source / "package.json", "package.json")
    ...
    ported_order = {
        f"{manifest['UUID']}.{suffix}": index
        for index, suffix in enumerate(PORTED_ACTION_SUFFIXES)
    }
    manifest["Actions"] = sorted(
        manifest["Actions"], key=lambda action: ported_order[action["UUID"]]
    )
    ...
    banner_references = manifest.get("Banner", [])
    if not isinstance(banner_references, list) or any(
        not isinstance(reference, str) for reference in banner_references
    ):
        raise ValueError("Manifest Banner must be an array of asset paths")
    asset_references = {
        manifest.get("Icon"),
    ...
    for inspector_name, expected_script in PROPERTY_INSPECTORS.items():
        inspector = exact_source_path(
            plugin_source, inspector_name, "property-inspector"
        )
        ...
        for source in parser.sources:
            ...
            for part in (*inspector_parent.parts, *source_path.parts):
                if part == "..":
                    if not parts:
                        raise ValueError(
                            "Property inspector script escapes the package"
                        )
                    parts.pop()
                elif part not in ("", "."):
                    parts.append(part)
            resolved_scripts.append(PurePosixPath(*parts).as_posix())
        expected_scripts = (*PROPERTY_INSPECTOR_VENDOR_FILES, expected_script)
