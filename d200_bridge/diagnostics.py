...
from .paths import CompanionPaths, load_token, validate_token
    ...
    for name in LOG_FILES:
        ...
        for line in data.decode("utf-8", "replace").splitlines():
            if len(line) > 1024:
                continue
            match = re.search(
                r"\b(INFO|ERROR) (companion_listening|startup_failed|redacted_event|"
                r"mediaremote_failure stage=(?:launch|timeout|exit|parse|schema) exit=(?:none|[0-9]{1,3}))\s*$",
                line,
            )
            if match:
