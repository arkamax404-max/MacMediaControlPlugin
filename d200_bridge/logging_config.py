...
class SafeEventFilter(logging.Filter):
    ...
    def filter(self, record):
        if (
            record.msg == "mediaremote_failure stage=%s exit=%s"
            and isinstance(record.args, tuple)
            and len(record.args) == 2
            and record.args[0] in MEDIAREMOTE_STAGES
            and (
                record.args[1] == "none"
                or isinstance(record.args[1], int)
                and 0 <= record.args[1] <= 255
            )
        ):
            record.msg = (
                f"mediaremote_failure stage={record.args[0]} exit={record.args[1]}"
            )
            record.args = ()
...
def configure_logging(
    log_directory, token=None, console=True, max_bytes=2 * 1024 * 1024
):
    ...
    file_handler = RotatingFileHandler(
        directory / "companion.log",
        maxBytes=max_bytes,
        backupCount=4,
        encoding="utf-8",
    )
    file_handler.__dict__["_d200_owned"] = True
    file_handler.setFormatter(formatter)
    file_handler.addFilter(SafeEventFilter())
    logger.addHandler(file_handler)
    if console:
        console_handler = logging.StreamHandler()
        console_handler.__dict__["_d200_owned"] = True
